...
export const TeacherTestQuestionMutate = () => {
  ...
  const myCourses = useMemo(
    () => allCourses.filter((c: Course) => c.teacher_id === user.id),
    [allCourses, user.id],
  );
  ...
  const myLessons = useMemo(
    () =>
      (lessonQuery.data ?? []).filter((l: Lesson) =>
        myCourseIds.has(l.course_id),
      ),
    [lessonQuery.data, myCourseIds],
  );
  ...
  const onSubmit = (data: FormData) => {
    ...
    const payload = isEdit
      ? {
          ...
          data: { ...data, id: id! } as TestQuestion,
        }
      : {
    ...
    mutation
      .mutateAsync(payload)
      .then((res) => navigate(`/teacher-questions/${res.id}`))
...
export default TeacherTestQuestionMutate;
